...
export default function ReceivedRequests({ receivedRequests, setReceivedRequests }) {
  return (
    <div>
          ...
          <div key={request._id} className="flex items-center justify-between p-3 border rounded-3xl">
            ...
            <div>
              <button
                ...
                onClick={() => handleAcceptRequest(request._id, setReceivedRequests)}
              >
                <FontAwesomeIcon icon={faCheck} />
              </button>
              <button
                className="bg-red-500 text-white p-2 rounded-full"
                onClick={() => handleRejectRequest(request._id, setReceivedRequests)}
              >
